...
def _eval_fields(ev: dict, cost: float, energy_tensor: np.ndarray | None = None) -> dict:
    ...
    return {
        ...
        "p_gs": None if ev.get("p_gs") is None else float(ev["p_gs"]),
    }
...
def run_trial(job: dict) -> dict:
    ...
    rec = {
        ...
        **inventory,
        ...
        "p_gs": final.get("p_gs"),
        "step_trace": list(getattr(result, "step_trace", []) or []),
    }
    if energy_tensor is not None:
...
def _summarize_group(recs: list[dict]) -> dict:
    ...
    pgs = [float(r["p_gs"]) for r in recs if r.get("p_gs") is not None]
    if pgs:
        out["mean_p_gs"] = float(np.mean(pgs))
    return out


def _summarize_by_hamiltonian(recs: list[dict]) -> dict:
    """Per-H mode-finding counts (most_likely_bitstring == ground)."""
    ...
    for hid, group in sorted(groups.items()):
        ...
        pgs = [float(r["p_gs"]) for r in group if r.get("p_gs") is not None]
        out[str(int(hid))] = {
            "n": n,
            "n_success": n_success,
            "success_rate": n_success / max(n, 1),
            ...
            "pick_p_gs": None if pick.get("p_gs") is None else float(pick["p_gs"]),
            "mean_energy_physical": float(np.mean(energies)) if energies else float("nan"),
            "mean_p_gs": float(np.mean(pgs)) if pgs else float("nan"),
        }
    return out
...
def run_saved_hamiltonian_suite(
    *,
    ...
    pgs = [float(r["p_gs"]) for r in records if r.get("p_gs") is not None]
    payload = {
        "method": method,
        "ansatz": ansatz,
        "family": family,
...
def run_mixed_p_spin_suite(
    *,
    ...
    return run_saved_hamiltonian_suite(
        ham_dir=ham_dir,
        n_trials=n_trials,
        outer_iter=outer_iter,
        spsa_iter=spsa_iter,
