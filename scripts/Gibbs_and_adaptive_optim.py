#!/usr/bin/env python3
"""Noiseless Gibbs VQE: joint SPSA on preparation and ECD ansatz.
...
2. Jointly SPSA-optimizes those five preparation variables **and** a random
   ECD ansatz for ``OUTER_ITERATIONS`` steps (default 70). Prep is not frozen.

``--spsa-iter N`` with ``N>0`` is an optional ablation: freeze prep after the
joint stage and continue the same ansatz vector for N more steps.

Gibbs η is always ``sampled_tail`` (histogram quantiles, no known \(E_{\min}\)).
...
from qumode_vqe.params import paper_bounds, random_parameters
...
def _print_job_line(i: int, n: int, rec: dict) -> None:
    ...
    print(
        f"[{i}/{n}] {sched_s}trial {rec['trial']}: {label}={rec['cost']:.4f}{extra}  "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--outer-iter",
        type=int,
        default=OUTER_ITERATIONS,
        help="Joint SPSA steps on (prep, ansatz). Default 70; prep stays live.",
    )
    parser.add_argument(
        "--spsa-iter",
        type=int,
